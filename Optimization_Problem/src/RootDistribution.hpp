...
// Note: Works for both Scaled & Unscaled EV
template <typename T>
std::vector<T> InitialRootDistr(const int NumEigVals, const int NumRoots, const int NumStages,
  ...
  // NOTE: Distribution with "additional root" in (0, 0) => Do NOT subtract 1 from NumRoots
  const T ArcLengthAVG = ArcLengths[NumEigVals-1] / (NumRoots - 0);
  
  std::vector<T> x0(NumRoots + 1); // +1: Include now also timestep
...
// Note: Works for both Scaled & Unscaled EV
template <typename T>
std::vector<T> InitialRootDistr(const int NumHullPoints, const int NumRoots, const int NumStages,
                                const T RealEigValMin, 
  ...
  for(size_t i = 1; i < NumHullPoints; i++) 
    ArcLengths[i] = ArcLengths[i-1] + Dist(HullRealScaled[i], HullImagScaled[i], HullRealScaled[i-1], HullImagScaled[i-1]);

  // Reasonable approximation for initial distribution of eigenvalues
  // NOTE: Distribution with "additional root" in (0, 0) => Do NOT subtract 1 from NumRoots
  const T ArcLengthAVG = ArcLengths[NumHullPoints - 1] / (NumRoots - 0);
