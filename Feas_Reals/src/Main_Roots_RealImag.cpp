...
int main(int argc, char** argv) {
   ...
   if(argc == 7)
      ...
      mynlp = new Roots_RealImag(NumStages, ConsOrder, NumStagesRef, dtRef, std::string(argv[5]), std::string(argv[6]));
   else
      mynlp = new Roots_RealImag(NumStages, ConsOrder, NumStagesRef, dtRef, std::string(argv[5]));

   // Create a new instance of IpoptApplication (use Ipopt::SmartPtr)
   SmartPtr<IpoptApplication> app = IpoptApplicationFactory();

   app->Options()->SetStringValue("output_file", "Roots_RealImag.out");

   // The following overwrites the default name (ipopt.opt) of the options file
   app->Options()->SetStringValue("option_file_name", "Roots_RealImag.opt");

   // Do no relaxation of bounds
   //app->Options()->SetNumericValue("bound_relax_factor", 1e-14);
   app->Options()->SetNumericValue("bound_relax_factor", std::numeric_limits<Number>::epsilon());

   if(ConsOrder == 1)
      // There are no equality constraints => constant Eq.-Constr. Jacobian
